...
'''
python3 batchMergerOneHotMaskLowEnergy.py --indir 2023_12_13_NtuplesV56/Input2/JetMET_PuppiJet_BarrelEndcap_Pt30_HoTot70 \
    --batchdir GoodNtuples --v HCAL --odir DataRecoOneHotMaskLowEnergy --filesPerRecord 50 --selectResp --fix_stats 100000 \
    ...
    with tf.device('/CPU:0'):
        if not options.rate_only:
            ...
            for blockIdx, block in enumerate(InFilesTrainBlocks):
                ...
                thr = 9
                print(" ### DEBUG: Masking all towers with {} energy < {}".format(options.v, thr))
                X[(X[:, :, 1] < thr), 1:41] = 0
                Energy = X[:,:,1]
                LowEnergySum = np.sum(np.where(Energy < thr, Energy, 0), axis=1)
                Y = Y - LowEnergySum

                def custom_one_hot_encoding(float_category): 
                    one_hot_encoded = np.zeros(199)
                    one_hot_encoded[np.digitize(float_category, np.arange(1,200)) - 1] = 1
                    return one_hot_encoded
                
                # One Hot Encode of the energy in bins np.arange(1,200)
                print(" ### DEBUG: Start One Hot Encoding")
                X_OneHot = np.apply_along_axis(custom_one_hot_encoding, axis=2, arr=X[:,:,1:2])
                print(" ### DEBUG: Start concatenating")
                X = np.concatenate([X[:,:,0:1], X_OneHot, X[:,:,2:]], axis=2)
                print(" ### DEBUG: End One Hot Encoding")
        ...
        if not options.noRate:
            ...
            for blockIdx, block in enumerate(InFilesRateBlocks):
                ...
                print(" ### DEBUG: Start One Hot Encoding")
                Z_OneHot = np.apply_along_axis(custom_one_hot_encoding, axis=2, arr=Z[:,:,1:2])
                print(" ### DEBUG: Start concatenating")
                Z = np.concatenate([Z[:,:,0:1], Z_OneHot, Z[:,:,2:]], axis=2)
                print(" ### DEBUG: End One Hot Encoding")
