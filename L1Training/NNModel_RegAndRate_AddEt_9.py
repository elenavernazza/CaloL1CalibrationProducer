# Regression + Regularization + Rate
# 5 network layers

'''
python3 NNModel_RegAndRate_AddEt_8.py --indir 2023_12_13_NtuplesV56/Input2/JetMET_PuppiJet_BarrelEndcap_Pt30_HoTot70 \
--v HCAL --tag DataReco --addtag _Test2A --epochs 10 --regr_w 50 --weig_w 1 --rate_w 1 --satu_w 1 --MaxLR 1e-04 --batch_size 256 --ThrRate 40
'''
...
def create_model(version, seedThr=None):
    ...
    layer3 = Dense(512, name='nn3',               activation='relu', kernel_initializer=RN(seed=7), use_bias=False)
    layer4 = Dense(256, name='nn4',               activation='relu', kernel_initializer=RN(seed=7), use_bias=False)
    layer5 = Dense(1,   name='nn5',               activation='relu', kernel_initializer=RN(seed=7), use_bias=False)
    layer6 = lay.Lambda(Fgrad)
        ...
        def weightsLoss():
            if options.weight_loss == 'abs':
                ...
                modelWeights_ss = float( tf.math.reduce_sum(tf.math.abs(modelWeights[0]), keepdims=True) +
                                        ...
                                        tf.math.reduce_sum(tf.math.abs(modelWeights[3]), keepdims=True) +
                                        tf.math.reduce_sum(tf.math.abs(modelWeights[4]), keepdims=True)
                                        )            
                ...
                modelWeights_ss = float( tf.math.reduce_sum(tf.math.square(modelWeights[0]), keepdims=True) +
                                        ...
                                        tf.math.reduce_sum(tf.math.square(modelWeights[3]), keepdims=True) +
