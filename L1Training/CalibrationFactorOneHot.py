...
from L1Training.NNModel_OneHotEnergy_RegAndRate_AddEt import *
    ...
    if reg == "HCAL" or reg == "ECAL":
        ...
        for i_energy in np.array(range(min_energy_tower, max_energy_tower+1, 1)):
            for i_eta in eta_towers:
                one_hot_tower = np.array([0 if i != i_energy else 1 for i in range(1,199+1)] + [0 if j != i_eta else 1 for j in range(1,40+1)])
                if reg == "ECAL":    
                ...
                input_energies.append(i_energy)
        input_towers = np.array(input_towers)
        Einput = np.array(input_energies).reshape(-1,1)
