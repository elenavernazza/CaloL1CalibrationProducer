...
if __name__ == "__main__" :
    ...
    TTP_output  = model_output[0]
    rate_output = model_output[1]
    OtherET     = model_output[2]
    ...
    rate_iesum = np.sum(RateTowers[:, :, [0, 1]], axis=2)
    rate_seed  = np.max(rate_iesum, axis=1)
    rate_et    = np.sum(RateTowers[:, :, 1], axis=1) + np.sum(RateTowers[:, :, 0], axis=1)
    jetPt_uncalib  = rate_et* (rate_seed > 4) / 2. # GeV
    jetPt_newcalib = rate_output / 2. # GeV

    binning = np.linspace(0,200,101)
    b_center = np.array((binning[:-1] + binning[1:])/2)
    e_uncalib, _   = np.histogram(jetPt_uncalib, bins=binning)
    e_newcalib, _  = np.histogram(jetPt_newcalib, bins=binning)
    ...
    plt.hist(jetPt_uncalib,  bins=binning, histtype='step', stacked=True, linewidth=2, label=leg_uncalib,  color=c_uncalib)
    plt.hist(jetPt_newcalib, bins=binning, histtype='step', stacked=True, linewidth=2, label=leg_newcalib, color=c_newcalib)
    ...
    h_off, _ = np.histogram(TargetPt, bins=binning)
    h_unc, _ = np.histogram(TargetPt[unCalib > thr_un], bins=binning)
    h_new, _ = np.histogram(TargetPt[newCalib > thr_new], bins=binning)

    fig = plt.figure(figsize = [10,10])
    plt.hist(TargetPt, bins=binning, label="Offline Inclusive", histtype='step', linewidth=2, color='Blue')
    text_1 = leg_uncalib + ": L1 jet Pt > {} Gev ".format(thr_un)
    plt.hist(TargetPt[unCalib > thr_un], bins=binning, label=text_1, histtype='step', linewidth=2, color=c_uncalib)
    text_3 = leg_newcalib + ": L1 jet Pt > {} Gev ".format(int(thr_new))
    plt.hist(TargetPt[newCalib > thr_new], bins=binning, label=text_3, histtype='step', linewidth=2, color=c_newcalib)
