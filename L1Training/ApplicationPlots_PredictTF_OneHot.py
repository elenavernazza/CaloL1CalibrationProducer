...
from NNModel_OneHotEnergy_RegAndRate_AddEt import *
...
if __name__ == "__main__" :
    ...
    def DeOneHotEncode (vec):
        if np.max(vec) == 0: return 0
        else: return np.argmax(vec) + 1

    rate_iem = RateTowers[:, :, 0]
    rate_ihad = np.apply_along_axis(DeOneHotEncode, axis=2, arr=RateTowers[:,:,1:199])
    rate_iesum = rate_iem + rate_ihad # iEt
    rate_seed  = np.max(rate_iesum, axis=1) # iEt
    rate_et    = np.sum(rate_iem, axis=1) + np.sum(rate_ihad, axis=1) # iEt
    ...
    rate_iem = RateTowers[:, :, 0]
    rate_ihad = np.apply_along_axis(DeOneHotEncode, axis=2, arr=Towers[:,:,1:199])
    rate_iesum = rate_iem + rate_ihad # iEt
    rate_seed  = np.max(rate_iesum, axis=1) # iEt
    rate_et    = np.sum(rate_iem, axis=1) + np.sum(rate_ihad, axis=1) # iEt
    jetPt_uncalib  = rate_et * (rate_seed > 8) / 2. # GeV
    ...
    iem = Towers[:, :, 0]
    ihad = np.apply_along_axis(DeOneHotEncode, axis=2, arr=Towers[:,:,1:199])
    unCalib  = (np.sum(iem, axis=1) + np.sum(ihad, axis=1)) / 2. # GeV
