...
def PlotTrainingTesting(x, y_train, y_test, loss_Name, odir):
    ...
    plt.savefig(odir+'/plots/'+loss_Name+'.pdf')
    plt.close()

def PlotAllLosses(x, y_reg, y_weight, y_rate, type, odir):
...
def makePlots(HISTORY, odir):
    ...
    PlotTrainingTesting(HISTORY['x'], HISTORY['train_weightsLoss'], HISTORY['test_weightsLoss'], 'Weights', odir)
    PlotTrainingTesting(HISTORY['x'], HISTORY['train_rateLoss'], HISTORY['test_rateLoss'], 'Rate', odir)
